...
Our seed corpus contains multilingual conversation templates tied to profile context and expert-style rubric labels. The processed training assets used in the repo include 84 core extractor-train examples, 48 dev, 48 test, a 140-example \texttt{extractor\_train\_best} split, 126/80/84 safety train/dev/test items, and 120/96/144 follow-up train/dev/test instances. We also added optional DAIC-WOZ export support as an \emph{auxiliary English-only} source, but did not let it replace the main multilingual benchmark because recent work shows DAIC-WOZ models can overfit therapist prompts instead of patient language \citep{burdisso2024daic}. The final live evaluation uses 48 held-out screening conversations with language-wise breakdowns for English, Hindi, and Hinglish. In the final written analysis, \emph{pure Hindi} examples are represented in \emph{Devanagari}; Roman script is reserved for explicitly code-mixed Hinglish behavior.
