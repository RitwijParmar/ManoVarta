...
OUT_DIR="/friend_root/reports/colab_daic_continue_vm/extractor_eval_smoke_rerun"
...
TS="$(date +%Y%m%d-%H%M%S)"
LOG="${RUN_DIR}/eval_localbase_${TS}.log"
PIDFILE="${RUN_DIR}/eval_localbase.pid"
INNER="${RUN_DIR}/docker_eval_inner.sh"
...
/opt/python/3.10/bin/python /workspace/tools/resumable_extractor_eval.py \
  ...
  --output-dir /friend_root/reports/colab_daic_continue_vm/extractor_eval_smoke_rerun \
  --max-new-tokens 900 \
  --device cuda \
  --use-4bit \
...
nohup sudo docker run --rm --gpus all \
  ...
  --entrypoint /bin/bash \
  "$IMAGE" \
  -lc "/workspace/docker_eval_inner.sh" \
  >"$LOG" 2>&1 &
