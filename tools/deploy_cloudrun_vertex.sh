...
VERTEX_LIVE_CHAT_ANALYSIS_LOCATION="${VERTEX_LIVE_CHAT_ANALYSIS_LOCATION:-global}"
VERTEX_LIVE_CHAT_ANALYSIS_FALLBACK_LOCATION="${VERTEX_LIVE_CHAT_ANALYSIS_FALLBACK_LOCATION:-${VERTEX_LOCATION}}"
CHAT_MODEL="${CHAT_MODEL:-gemini-3-flash-preview}"
CHAT_FALLBACK_MODEL="${CHAT_FALLBACK_MODEL:-gemini-2.5-flash}"
LIVE_CHAT_REPLY_MODEL="${LIVE_CHAT_REPLY_MODEL:-${CHAT_MODEL}}"
LIVE_CHAT_REPLY_FALLBACK_MODEL="${LIVE_CHAT_REPLY_FALLBACK_MODEL:-${CHAT_FALLBACK_MODEL}}"
LIVE_CHAT_ANALYSIS_MODEL="${LIVE_CHAT_ANALYSIS_MODEL:-gemini-3-pro-preview}"
LIVE_CHAT_ANALYSIS_FALLBACK_MODEL="${LIVE_CHAT_ANALYSIS_FALLBACK_MODEL:-gemini-2.5-flash}"
LIVE_CHAT_REPLY_TIMEOUT="${LIVE_CHAT_REPLY_TIMEOUT:-4}"
LIVE_CHAT_ANALYSIS_TIMEOUT="${LIVE_CHAT_ANALYSIS_TIMEOUT:-4}"
LIVE_CHAT_REPLY_THINKING_BUDGET="${LIVE_CHAT_REPLY_THINKING_BUDGET:-0}"
LIVE_CHAT_ANALYSIS_THINKING_BUDGET="${LIVE_CHAT_ANALYSIS_THINKING_BUDGET:-0}"
LIVE_CHAT_LLM_ANALYSIS="${LIVE_CHAT_LLM_ANALYSIS:-true}"
LIVE_LLM_TURN_THRESHOLD="${LIVE_LLM_TURN_THRESHOLD:-2}"
REMOTE_EXTRACTION_TIMEOUT="${REMOTE_EXTRACTION_TIMEOUT:-120}"
LIVE_CHAT_REMOTE_EXTRACTION_TIMEOUT="${LIVE_CHAT_REMOTE_EXTRACTION_TIMEOUT:-25}"
STARTUP_WARMUP="${STARTUP_WARMUP:-true}"
...
if [[ -n "${REMOTE_EXTRACTION_URL}" ]]; then
  ...
  EXTRACTION_REMOTE_ENV=",MANOVARTA_REMOTE_EXTRACTION_URL=${REMOTE_EXTRACTION_URL},MANOVARTA_REMOTE_EXTRACTION_TIMEOUT=${REMOTE_EXTRACTION_TIMEOUT},MANOVARTA_LIVE_CHAT_REMOTE_EXTRACTION_TIMEOUT=${LIVE_CHAT_REMOTE_EXTRACTION_TIMEOUT}"
else
...
if [[ "${CHAT_MODEL}" == gemini-* || "${CHAT_FALLBACK_MODEL}" == gemini-* || "${LIVE_CHAT_REPLY_MODEL}" == gemini-* || "${LIVE_CHAT_REPLY_FALLBACK_MODEL}" == gemini-* || "${LIVE_CHAT_ANALYSIS_MODEL}" == gemini-* || "${LIVE_CHAT_ANALYSIS_FALLBACK_MODEL}" == gemini-* || "${EXTRACTION_PROVIDER}" == "vertex" || "${LIVE_CHAT_EXTRACTION_PROVIDER}" == "vertex" ]]; then
  require_vertex_service_account_access
fi
